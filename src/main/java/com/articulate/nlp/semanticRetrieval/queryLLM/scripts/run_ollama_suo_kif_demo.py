...
from llm_types import DEFAULT_OLLAMA_PROMPT_VERBOSITY, OllamaGenerationConfig
from prompt_types import VALID_PROMPT_VERBOSITIES
from sentence_analyzer import DEFAULT_SPACY_MODEL


PROMPT = 'What sentence would you like to query with Ollama? (or "help" for options)'


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the Ollama SUO-KIF query demo."""

    parser = argparse.ArgumentParser(
        description=(
            "Run the full SUO-KIF grounding pipeline and query a local Ollama chat model."
        )
    )
    parser.add_argument(
        "sentence",
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Ollama generation model name.",
    )
    parser.add_argument(
        "--verbosity",
        choices=sorted(VALID_PROMPT_VERBOSITIES),
        default=DEFAULT_OLLAMA_PROMPT_VERBOSITY,
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Ollama API timeout in seconds. Default: %(default)s",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Ollama base URL used for embeddings and generation. Default: %(default)s",
    )
    parser.add_argument(
        "--spacy-model",
...
class OllamaDemoSession:
    """Interactive REPL for Ollama SUO-KIF querying."""
    ...
    def run(self) -> int:
        """Start the interactive Ollama loop."""
    ...
    def _query_and_print(self, sentence: str) -> None:
        ...
        result = self._orchestrator.query_ollama(
            trimmed_sentence,
    ...
    def _print_current_state(self) -> None:
        ...
        print("Ollama model: {0}".format(self.model))
        print("Prompt verbosity: {0}".format(self.verbosity))
        print("Ollama base URL: {0}".format(self.ollama_base_url))
    ...
    def _print_help(self) -> None:
        ...
        print("  set model=<ollama_model_name>")
    ...
    def _set_base_url(self, value: str) -> None:
        ...
        config = OllamaGenerationConfig(
            model=self.model,
            base_url=normalized_value or self.ollama_base_url,
            timeout_seconds=self.timeout_seconds,
        )
