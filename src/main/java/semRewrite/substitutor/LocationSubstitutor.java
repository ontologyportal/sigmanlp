...
public class LocationSubstitutor extends SimpleSubstitutorStorage {
    ...
    private Map<semRewrite.substitutor.CoreLabelSequence, semRewrite.substitutor.CoreLabelSequence> collectGroups(Annotation document) {
        ...
        while (rit.hasPrevious()) {
            ...
            if ("there".equals(label.lemma()) && ("RB".equals(label.tag()) || "VB".equals(label.tag()))) {
                if (rit.hasPrevious()) {
                    semRewrite.substitutor.CoreLabelSequence location = closestLocation(labels, rit.previousIndex());
                    if (!location.isEmpty()) {
                        collectedGroups.put(semRewrite.substitutor.CoreLabelSequence.from(label), location);
                    }
                }
            }
        }
