...
import static edu.stanford.nlp.dcoref.CorefCoreAnnotations.CorefClusterIdAnnotation;
...
public class StanfordCorefSubstitutor extends SimpleSubstitutorStorage {
    ...
    private void initialize(Annotation document) {
        ...
        Map<semRewrite.substitutor.CoreLabelSequence, semRewrite.substitutor.CoreLabelSequence> collectedGroups = Maps.newHashMap();
            ...
            if (mentions.size() > 1) {
                if (!ignorablePronouns.contains(label.originalText())) {
                    ...
                    if (sentenceIdx != firstMention.sentNum || index < firstMention.startIndex || index >= firstMention.endIndex) {
                        ...
                        semRewrite.substitutor.CoreLabelSequence key = extractTextWithSameTag(singleSentence, firstMention, masterTag);
                        if(!key.isEmpty()) {
     ...
     */
    private semRewrite.substitutor.CoreLabelSequence extractTextWithSameTag(List<CoreLabel> tokens, CorefMention mention, /* @Nullable */String masterTag) {
