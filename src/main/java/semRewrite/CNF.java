...
public class CNF {
    ...
    public ArrayList<semRewrite.Clause> clauses = new ArrayList<semRewrite.Clause>();
    ...
    public void clearBound() {
        ...
        ArrayList<semRewrite.Clause> newclauses = new ArrayList<semRewrite.Clause>();
        for (int i = 0; i < clauses.size(); i++) {
            semRewrite.Clause d = clauses.get(i);
            d.clearBound();
            if (!d.empty())
     ...
     */
    public semRewrite.CNF applyBindings(HashMap<String,String> bindings) {
     ...
     */
    private HashMap<String,String> unifyDisjunct(semRewrite.Clause d1, semRewrite.CNF cnf2, semRewrite.CNF cnf1, HashMap<String,String> bindings) {
     ...
     */
    public HashMap<String,String> unify(semRewrite.CNF cnf) {
    ...
    public static void testContains() {
        ...
        semRewrite.CNF cnf2 = semRewrite.CNF.parseSimple(lex2);
        ArrayList<semRewrite.CNF> al = new ArrayList<semRewrite.CNF>();
        al.add(cnf1);
        if (!al.contains(cnf2))
            al.add(cnf2);
