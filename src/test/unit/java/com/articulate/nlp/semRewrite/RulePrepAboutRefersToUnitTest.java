...
//    }
    ...
    public void testDecisionAbout() {
        ...
        String input = "root(ROOT-0, be-6), det(decision-2, the-1), " +
                "nsubj(be-6, decision-2), det(matter-5, the-4), " +
                "prep_about(decision-2, matter-5), det(committee-10, the-9), " +
                "advmod(be-6, up-7), prep_to(up-7, committee-10), " +
                "sumo(StateOfMind, up-7), sumo(Deciding, decision-2), " +
                "sumo(Commission, committee-10), sumo(Proposition, matter-5), " +
                "number(SINGULAR, decision-2), number(SINGULAR, matter-5), " +
                "tense(PRESENT, be-6), number(SINGULAR, committee-10)";
        ArrayList<CNF> cnfInput = interpreter.getCNFInput(input);
